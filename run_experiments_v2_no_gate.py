...
box (YCB SugarBox mesh), mustard bottle (YCB MustardBottle mesh,
...
feedback, 4 Aug -- see RIGOUR_LEDGER.md and MARKER_FEEDBACK.md):
  ...
  and the gripper's dynamic `hand` body tracks a separate mocap target
  via a weld equality constraint, so its momentum couples correctly
  through friction/contact to a grasped object (an earlier, rejected
  implementation drove `hand` itself as a mocap body -- mocap bodies
  contribute no rows to the contact Jacobian, so nothing was ever
  actually lifted; see RIGOUR_LEDGER.md Stage 21). `success` is a
  genuinely physical, independent outcome -- no longer a function of
  e_pose/q_grasp/n_grasps by construction.

RESTORATION NOTE (5 Aug): this file was accidentally deleted in commit
33cddb7 ("refactor: migrate to floating gripper demo and update grasp
config") the same day it was validated, and replaced with
demo_floating_gripper.py -- an interactive, single-object MuJoCo-viewer
*demo* (real-time, `mujoco.viewer.launch_passive`) that cannot run a
batch grid. There is no RIGOUR_LEDGER entry documenting that deletion
as an intentional decision; it looks like an unreconciled casualty of
building the viewer demo. Its absence was not noticed until a RunPod
batch run silently fell back to the OLD, deprecated run_experiments.py
(full-arm IK + `success = e_pose < 0.065` proximity criterion --
exactly the circularity Marker B flagged and this file's redesign was
built to eliminate) for an entire 1296-trial pilot run, because
`python run_experiments_v2_no_gate.py --test` failed with "No such file or
directory" and the natural fallback silently ran the wrong pipeline.
Restored here from git history (commit 3e688d7, the last version
before deletion) with the fixes below layered in, in response to that
incident.

NEW FIXES (5 Aug, response to RunPod pilot review):

  1. Explicit `seg_empty` status instead of a blind fallback. The old
     render_depth_seg (both here and in sim_common.py) silently
     substituted a `0.2 < depth < 1.5` mask whenever the true per-body
     segmentation came back empty -- this was traced to the phi=55/
     theta=0 "dead zone" pilot artifact: C_pc jumped from ~1e-5 to
     ~0.85 with no physically plausible camera-pose explanation,
     because the fallback was handing CGN a crop of the table, not the
     object. sim_common.render_depth_seg now always returns the TRUE
     segmentation and a `seg_empty` flag; this file short-circuits on
     `seg_empty` (skipping CGN entirely -- there is nothing to
     segment) and logs a distinct `no_visible_object` outcome, kept
     separate from `no_grasps` (CGN ran on a real crop and proposed
     nothing) in the failure taxonomy below.
  2. `C_pc` is now unconditionally computed from the true segmentation
     (point 1 makes this automatic -- there is no fallback mask left
     to contaminate it).
  3. `collision_free` was already logged; `contacted_bodies` (which
     body/bodies the open-finger pre-grasp pose actually touches) is
     now logged too, for direct diagnosis of collision failures
     without re-running the trial.
  4. `cube_size` -- the local-region crop size CGN's
     extract_3d_cam_boxes computes internally (previously visible only
     as a stdout print, which is how the phi=55 artifact above was
     root-caused in the first place) -- is now captured via
     estimator._last_cube_sizes and logged per trial as a first-class
     mediator.
  5. `failure_mode` taxonomy (sim_common.classify_failure_mode):
     success / no_visible_object / no_grasps / pregrasp_collision /
     executed_ejected (pushed sideways out of the gripper's footprint)
     / executed_dropped (stayed under the gripper but never sustained
     40% of the commanded lift). Each is a different terminal node in
     the SCM's selection structure and is not expected to share causes.

Usage:
    python run_experiments_v2_no_gate.py --object cylinder box mustard   # full grid, all objects (default)
    python run_experiments_v2_no_gate.py --object box                    # one object only
    python run_experiments_v2_no_gate.py --lean                           # reduced grid, faster first pass
    python run_experiments_v2_no_gate.py --test                           # tiny smoke test
    python run_experiments_v2_no_gate.py --resume                         # skip completed (object, trial_id) rows

Contact-GraspNet auto-selects CUDA if available (contact_grasp_estimator.py:
`self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")`).
Before a long run, confirm it will actually be picked up:
...
OUTPUT_CSV = os.path.join(RESULTS_DIR, 'experiment_results_v2_no_gate.csv')
...
XY_TOLERANCE_MARGIN = 0.03  # must match run_floating_gripper_test's default;
                            # kept explicit here since classify_failure_mode
                            # needs the same value to reproduce `tol`.
...
CSV_FIELDS = [
    ...
    'collision_free', 'contacted_bodies', 'gate_would_have_blocked', 'success',
...
def run_cgn(depth, K, seg_map, estimator, rho=1.0, rng=None):
    """
    Returns (pred_grasps, scores, cube_size). `cube_size` is the local-
    region crop size CGN computed internally for this trial's target
    segment (None if local_regions/use_cam_boxes didn't run, e.g. zero
    usable points survive extract_point_clouds) -- see
    contact_grasp_estimator.extract_3d_cam_boxes and the module
    docstring's fix #4.
    """
    if rng is None:
    ...
    # Single-object scenes use segment label 1 (see body_labels passed to
    # render_depth_seg below); fall back to the first recorded value if
    # the label scheme ever changes, rather than silently logging None.
...
def execute_grasp_floating(fg_scene_xml, spec, obj_pos, obj_quat, grasp_pos, grasp_quat):
    ...
    teleports the gripper to CGN's full 6-DoF predicted pose, and runs
    sim_common.run_floating_gripper_test() (collision check -> close
    fingers -> lift+shake -> check still held).
    """
...
def run_trial(trial_id, object_name, scene_xml, fg_scene_xml, sigma_d, rho, phi, theta, seed, estimator):
    ...
    # Always the true segmentation now (fix #1/#2 -- no fallback mask left
    # to inflate this): fraction of pixels genuinely showing the target.
    ...
    if seg_empty:
        # Object not visible from this viewpoint at all -- a legitimate
        # causal outcome of (phi, theta, sigma_d, rho), not an error to
        # paper over with a fallback mask. CGN is never run: there is
        # nothing for it to segment, and running it on a fabricated
        # scene-spanning crop is exactly the bug this fix removes.
        ...
        return {**row_base, 'C_pc': round(C_pc, 5), 'cube_size': None,
                ...
                'collision_free': None, 'contacted_bodies': None, 'gate_would_have_blocked': None, 'success': 0,
                'final_xy_offset': None, 'final_lift': None, 'obj_z_final': None,
    ...
    return {**row_base, 'C_pc': round(C_pc, 5), 'cube_size': cube_size,
            'q_grasp': round(q_grasp, 5), 'e_pose': round(e_pose, 5), 'n_grasps': n_grasps,
            'collision_free': int(result['collision_free']),
            'gate_would_have_blocked': int(not result['collision_free']),
            ...
            'obj_z_final': result['obj_z_final'],
...
def main():
    ...
    parser.add_argument('--subset', type=int, help='Run only a random subset of N trials per object')
    ...
    if args.subset:
        import random
        random.seed(42)
        trials = random.sample(trials, min(args.subset, len(trials)))
    ...
    for object_name in args.object:
        ...
        for trial_id, sigma_d, rho, phi, theta, seed in trials:
            ...
            try:
                ...
                print(f'{s:<18}  z={row.get("obj_z_final","?")}  score={row.get("q_grasp","?")}  '
