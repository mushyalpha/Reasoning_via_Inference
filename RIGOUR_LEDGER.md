...
## Stage 23 — run_experiments_v2.py accidentally deleted, ran deprecated script on RunPod, restored + hardened (5 Aug)

- **Incident:** `run_experiments_v2.py` (Stage 21's validated floating-gripper batch runner) was deleted the same day it was validated, in commit `33cddb7` ("refactor: migrate to floating gripper demo and update grasp config"), which added `demo_floating_gripper.py` (an interactive, single-object, real-time MuJoCo-viewer *demo* -- `mujoco.viewer.launch_passive`, wall-clock sleeps per frame -- with no batch-grid capability) in its place. No RIGOUR_LEDGER entry reconciled this; it appears to be an unreconciled casualty of building the viewer demo, not a deliberate decision. Twenty minutes later, commit `1f27207` densified the experimental grid -- but edited the OLD `run_experiments.py` (never deleted, still using the pre-Stage-21 `success = e_pose < 0.065` proximity criterion with a HARDCODED `obj_z_final` on success) rather than recreating `run_experiments_v2.py`. On RunPod, `python run_experiments_v2.py --test` correctly failed ("No such file or directory"); the natural fallback to `python run_experiments.py --test` succeeded and silently ran the deprecated proximity-based script for an entire 1296-trial pilot batch. The resulting `obj_z_final` bimodality (0.455 resting / 0.675 "lifted") was initially mistaken, during review of that pilot run, for genuine floating-gripper physics -- it is not: `0.675` is `CYLINDER_Z + 0.22`, a literal constant written whenever the proximity check passes, not a measured outcome. This was only caught by reading `execute_grasp()`'s source directly, not by any output-level check (the same category of error Stage 21's mocap-vs-weld bug fell into: outcome inspection alone was not sufficient).
- **Consequence for the pilot data:** the entire RunPod 1296-trial run must be treated as measuring the pre-Stage-21 proximity criterion, not the floating-gripper protocol. Two structural findings from reviewing that pilot remain valid and are addressed below because they are properties of shared code (`sim_common.render_depth_seg`, `contact_grasp_estimator.extract_3d_cam_boxes`) used by both the deprecated and the correct pipeline: (i) a blind depth-range segmentation fallback (`if seg_map.sum()==0: seg_map = (0.2<depth<1.5)`) was traced, via the now-recorded region-cube-size, to fabricating a scene-spanning crop at specific (phi>=60, theta=0) combinations where the true segmentation returns zero pixels -- C_pc jumped from ~1e-5 to ~0.85 with no physically plausible camera-pose explanation; (ii) CGN's own `regularize_pc_point_count` draws from the *global* `np.random` (not the caller's seeded `np.random.default_rng(seed)`), so trials that only differ by seed at sigma_d=0/rho=1 were not actually reproducible before `sim_common.seed_cgn_global_random` (already written, just never wired into the script that actually ran on RunPod).
- **Restoration + new fixes:** `run_experiments_v2.py` recovered from git history (commit `3e688d7`, the last version before deletion -- confirmed compatible with the current `object_specs.py`/`sim_common.py` APIs with no changes needed there) and hardened:
  1. `sim_common.render_depth_seg` now always returns the TRUE segmentation (the blind fallback is removed entirely) plus an explicit `seg_empty` flag; `run_experiments_v2.py`/`run_clutter_experiments.py` short-circuit on `seg_empty` (CGN is never run on a fabricated crop) and log a distinct `no_visible_object` outcome, separate from `no_grasps` (CGN ran on a real crop and proposed nothing).
  2. `C_pc` is therefore unconditionally the true visible-fraction now -- no fallback mask left to inflate it.
  3. `contacted_bodies` (which body the open-finger pre-grasp pose actually touches) is now logged alongside the pre-existing `collision_free`.
  4. `contact_grasp_estimator.extract_3d_cam_boxes` now records its computed region cube size on `self._last_cube_sizes` (previously visible only as a stdout print -- how the phi=55/theta=0 artifact above was root-caused in the first place); logged per trial as `cube_size`.
  5. New `sim_common.classify_failure_mode` taxonomy, logged as `failure_mode`: `success` / `no_visible_object` / `no_grasps` / `pregrasp_collision` / `executed_ejected` (pushed sideways out of the gripper's footprint) / `executed_dropped` (stayed under the gripper but never sustained 40% of the commanded lift) -- each a distinct terminal node in the SCM's selection structure.
  6. The RunPod-only lazy-import fix for `contact_graspnet_pytorch/data.py` (moving `from contact_graspnet_pytorch.scene_renderer import SceneRenderer` out of module scope, since `pyrender`'s eager `OpenGL.platform.egl` import crashes on machines without a working EGL stack even when `SceneRenderer` is never used) had only ever been applied live on the RunPod filesystem, never committed -- it is now in the repo, so this is not silently lost again the next time a fresh machine is provisioned.
  7. `run_experiments.py` (the deprecated script) now carries an explicit deprecation header naming this exact incident, so a future missing-`run_experiments_v2.py` situation is caught before a batch run rather than after.
- **Validated:** `--test` smoke runs for all three objects (cylinder/box/mustard) on a fresh machine (no prior CGN/mujoco install) complete without error; genuine physical lifts are observed (e.g. mustard z rising from 0.484 resting to 0.6334, cylinder 0.455 to 0.601 -- not the old hardcoded 0.675); `pregrasp_collision`, `executed_dropped`, `executed_ejected`, and `success` all appear in an 8-16 trial sample, confirming the taxonomy actually discriminates; a repeated `--test --object cylinder` run reproduced a byte-identical CSV, confirming the seeding fix is genuinely wired in this time (not just written and left unused, as happened before this incident).
- **Holds?** fixed (was: silently broken) -- for Experiment A's batch runner and its dependency on `sim_common`/CGN. `run_clutter_experiments.py` (Experiment B) received the same signature-compatibility and taxonomy updates as a consequence (it shares the exact same `render_depth_seg`/`extract_3d_cam_boxes` code paths and would otherwise have broken on the `render_depth_seg` return-signature change), but has not yet been re-run or re-validated with a fresh `--test` pass -- only Experiment A's smoke test was actually executed.
- **Severity** high (this is the same "gets the SCM-vs-LLM comparison right or not" outcome variable Stage 21 already rated high; the failure mode here was procedural -- running the wrong file -- rather than a modelling error, but the consequence for data validity is identical).
- **Disposition** fix-now, closed for Experiment A's runner; carrying forward as fix-now for Experiment B: (1) run `run_clutter_experiments.py --test` on a machine with the full dependency set (this session's validation only reached Experiment A) before trusting it; (2) re-run the executor-symmetry check proposed alongside this review (rotate a known-good cylinder pose in 15° increments through the floating-gripper test only, no CGN, no arm) before treating any theta effect in a future confirmatory run as a genuine viewpoint moderator rather than a shake-trajectory/jitter-axis artifact; (3) run the full (non-`--test`) `run_experiments_v2.py` and `run_clutter_experiments.py` batches on RunPod now that the outcome mechanism, the segmentation fallback, and the seeding gap are all fixed in the file that will actually run.
- **Evidence / notes:** `run_experiments_v2.py` (restored + fixed), `run_clutter_experiments.py`, `sim_common.py` (`render_depth_seg`, `classify_failure_mode`), `contact_graspnet_pytorch/contact_graspnet_pytorch/contact_grasp_estimator.py` (`_last_cube_sizes`), `contact_graspnet_pytorch/contact_graspnet_pytorch/data.py` (lazy `SceneRenderer` import), `run_experiments.py` (deprecation header). Git history: `9eab530` (original v2 script), `3e688d7` (weld-constraint migration, restoration base), `33cddb7` (accidental deletion), `1f27207` (grid densification landed on the wrong file).
...
## Counts at last review (updated 5 Aug, fourth pass — Stage 23 added: run_experiments_v2.py accidentally deleted, ran deprecated proximity-criterion script on RunPod, restored + hardened)

- Total tracked stages: 23
- `Holds? = yes`: 0 · `partial`: 10 · `open`: 5 · `challenged`: 2 · `fixed`: 2 · `superseded`: 4
- `Disposition`: keep-for-MSc: 6 · fix-now: 11 (2 closed: Stage 7, and Stage 9/10-threshold-sweep from 3 Aug; 1 newly closed: Stage 23 for Experiment A, carried forward for Experiment B) · future-work: 3 · redesign-candidate: 5 (3 newly actioned: Stages 5, 8, 11 — code built, not yet run)
