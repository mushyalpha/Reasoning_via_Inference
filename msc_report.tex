...
This thesis develops and audits a pre-registered and revised mechanistic Structural Causal Model (SCM) for diagnosing robotic grasp failure. Given a failed grasp attempt, we ask whether Pearl's three-step counterfactual procedure---abduction, action, and prediction---applied to an SCM derived from the known dataflow of the perception-action pipeline, can correctly attribute the failure to its environmental root cause when a single cause exists, and correctly recognise the failure as multi-cause or irreducible when a single cause does not suffice. Because the perturbations are experimenter-controlled, the true cause of each failure is always available for scoring against simulated ground truth, making this a falsifiable methodological audit rather than an unfalsifiable post-hoc narrative. As a secondary comparison, we contrast this structured diagnosis against an LLM baseline (a VLM-capable model given text only, not images) supplied with identical observational data, to quantify what the SCM's explicit structural knowledge of the pipeline contributes relative to associative natural-language reasoning.

The SCM links four independently controllable perceptual variables---depth noise~($\sigma_d$), point cloud sparsity~($\rho$), viewpoint elevation~($\phi$), and viewpoint azimuth~($\theta$)---through measurable intermediate variables (point cloud completeness, the number of grasp candidates, grasp confidence, and pose error) to binary grasp success. The approach is implemented in MuJoCo with Contact-GraspNet as the grasp synthesis model, injecting synthetic Gaussian depth noise directly onto the simulated depth buffer to obtain perturbations that are causally clean and independently controlled, a level of experimental precision that is difficult to achieve on physical hardware. In an initial 432-trial pilot spanning a full factorial grid over the four variables, scored under a proximity-based success criterion (Section~\ref{sec:success_criterion_detail}), depth noise emerged as the dominant cause of failure---grasp success fell monotonically from 70.4\% under a noise-free sensor to 0.0\% under the most severe noise condition, with viewpoint elevation and point cloud sparsity producing smaller effects that interact with depth noise rather than acting independently. This descriptive finding motivated the physically-grounded floating-gripper redesign (Section~\ref{sec:approach}) on which the confirmatory results of this thesis are based. On the proximity pilot, the two diagnosticians are compared on attribution accuracy, consistency across repeated queries, and explanation quality; the confirmatory floating-gripper study reports the physical-outcome decomposition rather than a re-scored SCM/LLM bake-off.
...
Vision-language models can describe a failed trial in natural language and propose a plausible cause, but their explanations are associative rather than structural, and are not tested against what would actually have happened under intervention. Chapter~\ref{ch:background} reviews these approaches in detail. What remains absent is an audit method that (a)~leverages the known computational architecture of the robot to build a structural model of failure and (b)~validates that model using a principled counterfactual test: \emph{if this particular structural precondition had been met, would the grasp have succeeded?} Figure~\ref{fig:diagnostic_gap} situates these approaches on the Pearl Causal Hierarchy, making the diagnostic gap geometrically precise: only a match between model assumptions and query expressiveness yields valid inference.
...
Given an observed grasp failure, we apply Pearl's three-step counterfactual procedure~\cite{pearl2018book}---\emph{abduction}, \emph{action}, and \emph{prediction}---to identify which modelled environmental factor most likely caused the failure. Because the exogenous settings ($\sigma_d$, $\rho$, $\phi$, $\theta$) are logged under full experimental control rather than inferred, abduction here recovers only the unobserved noise terms of the intermediate structural equations, not the settings themselves; the diagnostic work is carried by the action and prediction steps, which ask what would have happened to \emph{that specific failed trial} under a different setting of one variable. This remains an $\mathcal{L}_3$ query---it conditions on the realised outcome, which a purely interventional model cannot do---and Section~\ref{sec:counterfactual_diagnosis} states this formally. Because the testbed directly implements the do-operator, we are further able to rewind the simulation and verify each such counterfactual against ground truth: the trial's random seed is recorded and held fixed on rewind, so that only the intervened-on variable changes and every other stochastic element of the pipeline replays identically. This is abduction carried out physically rather than by solving an equation---the recorded seed \emph{is} the abducted noise realisation---and it is what licenses reading the re-simulated outcome as the true counterfactual for that specific trial, establishing whether fixing the identified perceptual variable would have saved the grasp, or whether the failure was irreducibly constrained by physical geometry.
...
Chapter~\ref{ch:results} reports the descriptive, decomposition, and counterfactual findings, including the collision-filtering ceiling check and the quantification of multi-cause and irreducible failures.
...
\paragraph{Pre-registered versus discovered graphs.} Section~\ref{sec:bg_diagnosis} distinguished two ways a graph can be obtained: discovered from data---by statistical causal discovery, or by structure search as in Diehl and Ramirez-Amaro~\cite{diehl2022whydidifail}---or pre-registered from known architecture. This thesis takes the second path throughout: Chapter~\ref{ch:formulation} states the graph of Figure~\ref{fig:causal_dag} before any diagnostic use is made of it, and Section~\ref{sec:falsification} reports the pre-specified tests that could have, and in two separate episodes did, contradict it. Pre-registration does not make the graph immune from being wrong; it makes being wrong a testable event rather than an unfalsifiable modelling choice.

\paragraph{Parametric versus non-parametric SCMs, and what this thesis commits to.} Vonk et al.~\cite{vonk2023disentangling} distinguish SCMs specified \emph{non-parametrically}, where each $f_i$ is left as an arbitrary function and only qualitative properties (e.g.\ monotonicity) are assumed, from SCMs specified \emph{parametrically}, where a functional form is fixed in advance. Non-parametric specification is preferable when feasible, because it avoids functional-form misspecification, but arbitrary functions cannot be estimated from finitely many trials without additional structure. This thesis separates two decisions that are often bundled together, and commits to a different degree of specificity for each. The \emph{graph} $\mathcal{G}$---which variables are causes of which---is pre-registered from the pipeline's known computational architecture, as just discussed; it is not discovered, and it is not left unspecified, because the dataflow itself settles it (Section~\ref{sec:causal_graph}). The \emph{mechanisms} $f_i$ (Section~\ref{sec:structural_equations}) are a different kind of object and are deliberately given far less commitment: they are low-order, linear-in-parameters regressions chosen for interpretability, not because the true mechanism---a deep network's forward pass---is believed to be linear in any strong sense, and they are not stretched to explain every non-linearity visible in the data. Two consequences follow directly from taking this stance seriously rather than stating it and then fitting a single all-purpose regression per node regardless. First, the count variable $n_{\text{grasps}}$ has a point mass at zero---Contact-GraspNet returns no candidate at all on $141/432$ trials, concentrated at severe noise---which a linear-Gaussian equation cannot represent and which is not merely a distributional nuisance: ``CGN found nothing to grasp'' and ``CGN found a corrupted pose'' are mechanistically distinct events with plausibly different causes (severe noise destroys the point cloud outright; moderate noise degrades an existing proposal), and pooling them into one continuous residual would blur exactly the distinction the diagnosis in Chapter~\ref{ch:diagnosis} depends on. This thesis instead models the count as a two-part \emph{hurdle}: a binary gate $\text{has\_grasps}$ fitted by logistic regression, and $n_{\text{grasps}}$ fitted as a count model on the sub-population that clears the gate (Section~\ref{sec:structural_equations}). Second, wherever a specific functional form was found to overreach---most notably the $q_{\text{grasp}} \to e_{\text{pose}}$ term, which Section~\ref{sec:causal_graph} shows is not a mediation edge at all once the pipeline's dataflow is read directly---the term is removed rather than kept and explained away. To keep the surrogate honest rather than assumed, every remaining equation's linear-in-parameters prediction is checked against a \emph{nonparametric}, model-free estimate---stratified means and Wilson-score proportions computed directly on the factorial grid, with no line fitted to them (Section~\ref{sec:falsification}, and \texttt{results/scm\_nonparametric\_report.md} in full)---and any divergence between the two is reported as a located failure of the surrogate rather than smoothed into the residual or used to justify a more elaborate basis function. The graph, in short, is the pre-registered, falsifiable object that this thesis's diagnostic claims rest on; the mechanisms are a deliberately under-specified, cross-checked surrogate for that graph's edges, not a second layer of commitment carrying equal evidentiary weight.
...
Contact-GraspNet (CGN)~\cite{sundermeyer2021contact} is a neural network for 6-DoF grasp generation from point-cloud observations: given a partial point cloud, it outputs a set of grasp proposals, each a $4\times4$ pose together with a scalar confidence, by predicting per-point contact probability rather than regressing poses from global point cloud features. Trained on synthetic ShapeNet scenes under clean sensing conditions (no depth noise, no extreme viewpoint), CGN's headline accuracy---over 90\% on unseen objects in structured clutter---is reported downstream of a collision-aware selection stage that filters candidate grasps before execution.

That filtering stage is frequently left implicit in how such numbers are quoted, yet it is itself a precondition of success rather than a detail: without it, poses that are in collision with the object or support surface are counted alongside geometrically valid ones, and success rates collapse for reasons that have nothing to do with perception. This observation motivates the two-arm design of Chapter~\ref{ch:method} (Section~\ref{sec:approach}), which separates an unfiltered arm (raw geometric validity) from a filtered arm (perception under audit) precisely so that this often-implicit precondition is measured rather than assumed away.
...
The exogenous variables are the environmental factors that the experimenter sets directly in each trial, before any perception occurs. Two of them describe the quality of the depth measurement: $\sigma_d$, the standard deviation of Gaussian noise injected per pixel onto the simulated depth buffer, and $\rho$, the fraction of points retained after random downsampling of the resulting point cloud. The other two describe where the observation is made from: $\phi$, the camera's elevation angle above the horizontal plane, and $\theta$, its azimuth, both defined on a fixed-radius sphere centred on the target object. Together these operationalise sensor degradation and partial observability, respectively: for example, a trial with a large $\sigma_d$ and a small $\rho$ presents Contact-GraspNet with a sparse, geometrically corrupted point cloud, while a trial at a steep $\phi$ presents it with a clean cloud that nonetheless conceals the object's vertical surfaces. Because the experiment is a full factorial design, every combination of these four values is assigned rather than sampled, so they are mutually independent by construction and have no parents in the causal graph of Section~\ref{sec:causal_graph}.

The endogenous variables are measured or computed within each trial and are not set directly by the experimenter. Point cloud completeness $C_{pc}$ is the fraction of image pixels belonging to the target object, computed from the simulator's segmentation buffer; it measures how much of the object is geometrically visible from the current viewpoint, so a low $C_{pc}$ means the object is poorly resolved in the image. The name is a slight misnomer worth flagging once: $C_{pc}$ is a 2-D segmentation-mask pixel fraction computed \emph{before} the 3-D point cloud is formed, which is precisely why depth noise and downsampling---both point-cloud-stage manipulations---turn out to have no causal effect on it (Section~\ref{sec:falsification}). The name is retained for continuity with the codebase and prior sections of this thesis rather than introduced fresh here. The remaining variables characterise Contact-GraspNet's output. The gate $\text{has\_grasps} \in \{0,1\}$ records whether CGN returns any grasp candidate at all on this trial's point cloud; $n_{\text{grasps}}$ is the number of candidates returned; $q_{\text{grasp}}$ is the confidence score CGN assigns to the top-ranked candidate, so a higher value means CGN regards the grasp it proposes as more likely to succeed; and $e_{\text{pose}}$ is the Euclidean distance between that proposed gripper position and the true object centroid, so a large $e_{\text{pose}}$ means the pipeline has proposed a grasp that is spatially displaced from the object it is meant to grasp. The outcome variable $Y \in \{0,1\}$ is binary grasp success, operationalised in Section~\ref{sec:success_criterion_detail}. The gate is treated separately from the other three because when $\text{has\_grasps}=0$ the pipeline produces no pose at all, and $n_{\text{grasps}}$, $q_{\text{grasp}}$ and $e_{\text{pose}}$ are undefined for that trial; this is a qualitatively different failure from producing a poor pose, and it has a qualitatively different downstream consequence.

Finally, a terminological clarification. This thesis uses ``exogenous'' in two related but distinct senses. In the graphical sense, $\sigma_d$, $\rho$, $\phi$, $\theta$ are exogenous because they have no parents in the DAG. Separately, each structural equation in Section~\ref{sec:structural_equations} carries its own exogenous \emph{noise term} $U_i$ (e.g.\ $U_C$, $U_q$, $U_n$, $U_e$), representing unmodelled factors internal to that mechanism, such as segmentation noise, CGN's internal stochasticity, or pose-regression error. It is the independence properties of these noise terms, not of the four controlled variables, that determine whether the model is Markovian in the sense required by the counterfactual identification argument of Section~\ref{sec:counterfactual_diagnosis}.

\section{Causal Graph}
\label{sec:causal_graph}

The causal relationships between the variables are encoded in the directed acyclic graph shown in Figure~\ref{fig:causal_dag}. This graph was derived by reading the pipeline's own execution order (each edge below is justified as ``the code that computes $Y$ reads the value of $X$'', following the dataflow procedure motivated in Section~\ref{sec:bg_scm}), not from inspecting \texttt{experiment\_results.csv}; the resulting edges and non-edges were only afterwards checked interventionally against the data (Section~\ref{sec:falsification}). The graph reflects the following causal assumptions:
...
\begin{enumerate}
  ...
  \item The gate $\text{has\_grasps}$---whether Contact-GraspNet returns any candidate at all---is caused directly by all four exogenous variables ($\sigma_d, \rho, \phi, \theta$), and \emph{not} mediated through $C_{pc}$. This is the primary causal claim of the SCM. It is not mediated through $C_{pc}$ because CGN's forward pass reads the full 3-D point cloud, which carries $\sigma_d$ (depth-buffer noise) and $\rho$ (downsampling) as well as viewpoint, whereas $C_{pc}$ is only the 2-D pixel-coverage fraction computed from the segmentation mask; $\phi,\theta$ therefore have two separate downstream paths (one to $C_{pc}$, one to $\text{has\_grasps}$) rather than one path routed through the other.

  \item Conditional on $\text{has\_grasps}=1$: the count $n_{\text{grasps}}$ is caused by all four exogenous variables; $q_{\text{grasp}}$ is caused by all four exogenous variables directly, plus $n_{\text{grasps}}$ (an order-statistic effect: the reported score is the maximum over a pool of size $n_{\text{grasps}}$, so a larger pool mechanically raises the expected maximum independently of any causal improvement in individual candidates); and $e_{\text{pose}}$ is caused by all four exogenous variables directly, through the same camera-to-world transform that determines which surfaces are visible and where the proposed pose lands.

  \item There is \emph{no} $q_{\text{grasp}} \to e_{\text{pose}}$ edge, despite the two varying together in the data. Both are read from the same $\arg\max$ index of the same underlying set of scored candidates that Contact-GraspNet returns on a given trial; the code that computes the pose never reads the confidence score. $q_{\text{grasp}}$ and $e_{\text{pose}}$ are therefore \emph{siblings under a common (unobserved) parent}---the full set of scored candidates---correlated through a shared selection index rather than through either causing the other. An earlier draft of this graph, and of the corresponding structural equation (Section~\ref{sec:structural_equations}), treated $q_{\text{grasp}}$ as a mediator of $e_{\text{pose}}$ and reported a ``percentage mediated''; Section~\ref{sec:falsification} reports the test that shows this framing is not supported. This is a concrete, dataflow-provable instance of the independent-errors (NPSEM-ie) assumption failing for one specific pair of variables, which Section~\ref{sec:counterfactual_diagnosis} returns to.

  \item $\text{has\_grasps}=0$ forces $Y=0$ directly: with no candidate returned, there is no pose to execute, independently of every other variable. Conditional on $\text{has\_grasps}=1$, $Y$ is governed by whether the executed pose is close enough to the object, operationalised in Section~\ref{sec:success_criterion_detail}.
\end{enumerate}

\begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{results/figures/causal_dag.pdf}
  \caption{Causal directed acyclic graph (DAG), pre-registered (\texttt{CAUSAL\_DAG\_PREREGISTRATION.md}) and corrected after the falsification tests of Section~\ref{sec:falsification}. $\sigma_d$ and $\rho$ have no direct edge into $C_{pc}$; $\text{has\_grasps}$ (whether CGN returns any candidate) is the primary causal node and is not mediated through $C_{pc}$; $q_{\text{grasp}}$ and $e_{\text{pose}}$ are siblings with correlated errors (dashed, non-causal), not a mediation chain; $\text{has\_grasps}=0$ forces $Y=0$ directly.}
...
The mechanisms below are parameterised as a small set of regression models, chosen for interpretability rather than to represent the true mechanism exactly---only the graph of Section~\ref{sec:causal_graph} carries the pre-registered, falsifiable commitment that this thesis's diagnostic claims depend on (Section~\ref{sec:bg_scm}). Two structural decisions follow directly from taking that separation seriously, rather than fitting one all-purpose linear or logistic equation per node regardless of what the variable actually looks like. First, $n_{\text{grasps}}$ has a point mass at zero---CGN returns no candidate at all on 141 of 432 trials---so it is modelled as a two-part \emph{hurdle} rather than as one continuous equation: a gate $\text{has\_grasps}$ (Equation~\eqref{eq:hg}), and a count conditional on the gate (Equation~\eqref{eq:ng}). This is not a distributional nicety; ``CGN found nothing to grasp'' and ``CGN found a corrupted pose'' are mechanistically distinct events with plausibly different causes (severe noise destroys the point cloud outright, moderate noise degrades an existing proposal), and a single continuous equation would blur exactly the distinction the diagnosis in Chapter~\ref{ch:diagnosis} depends on. Second, $q_{\text{grasp}}$ and $e_{\text{pose}}$ are specified as siblings, not as a mediation chain (Section~\ref{sec:causal_graph}, Assumption 4): both are caused by the four exogenous variables directly, and neither equation includes the other as a regressor.

Within each equation, the functional form itself is kept deliberately plain: low-order polynomial terms in the exogenous variables, with no basis function retained unless it earns its place empirically. Every equation's prediction is checked against a nonparametric, model-free estimate computed directly on the factorial grid---no line fitted to it---and reported in Section~\ref{sec:falsification} and in full in \texttt{results/scm\_nonparametric\_report.md}; where the two diverge, as they do for the CGN pose bias discussed after Equation~\eqref{eq:ep}, this is reported as a located failure of the surrogate rather than absorbed into a higher-order term added after the fact to make the fit look better. Investigating flexible (e.g.\ spline or neural) parameterisations of these equations, which would relax the linearity assumption at the cost of interpretability and data efficiency, remains identified as future work in Chapter~\ref{ch:discussion}; the position taken here is that a scoped, checked, and readily-falsified linear surrogate is preferable to a more flexible one that this dataset cannot support without overfitting. The dataset comprises 432 trials in total; six (1.4\%) terminated with a file-locking race condition in the logging layer and have no recorded outcome (Section~\ref{sec:limitations}), leaving 426 with a usable outcome for every equation fitted below.
...
\begin{equation}
  C_{pc} = \beta_0 + \beta_1 \phi + \beta_2 \theta + U_C,
  \label{eq:cpc}
\end{equation}
where $U_C$ is the exogenous noise term representing all other factors that affect point cloud completeness but are not modelled. Depth noise $\sigma_d$ and sparsity $\rho$ do not appear in this equation: an earlier specification included both as predictors, but the pre-specified falsification test in Section~\ref{sec:falsification} found no measurable partial effect of either variable on $C_{pc}$, consistent with $C_{pc}$ being computed from the segmentation buffer before noise injection and downsampling occur in the pipeline. The equation was revised accordingly rather than retaining the terms and absorbing the null effect into $U_C$. A trigonometric transform of $\phi$ (e.g.\ $\sin\phi$) was considered, on the grounds that visible-area fraction should saturate at high elevation rather than continue linearly, but is not used here: the plain linear term already accounts for 89.3\% of the variance in $C_{pc}$ over the sampled range ($\phi \in [30°, 60°]$; Section~\ref{sec:scm_fit}), and adding a basis function the data do not need would be exactly the kind of unwarranted specificity Section~\ref{sec:bg_scm} argues against.

\subsection{Grasp Gate: Does CGN Return Any Candidate?}
\label{sec:eq_gate}

This is the primary causal claim of the SCM: whether Contact-GraspNet returns a usable candidate at all, as distinct from how good that candidate is once one exists. It is modelled as a logistic gate,
\begin{equation}
  P(\text{has\_grasps} = 1) = \sigma\!\left(\eta_0 + \eta_1 \sigma_d + \eta_2 \rho + \eta_3 \phi + \eta_4 \theta\right),
  \label{eq:hg}
\end{equation}
with all four exogenous variables entering directly rather than through $C_{pc}$ (Section~\ref{sec:causal_graph}, Assumption 2). $\text{has\_grasps} = 0$ forces $n_{\text{grasps}} = 0$ and leaves $q_{\text{grasp}}, e_{\text{pose}}$ undefined for that trial; downstream of the gate, success itself is decomposed as $P(Y{=}1) = P(\text{has\_grasps}{=}1) \times P(Y{=}1 \mid \text{has\_grasps}{=}1)$, so that ``no proposal was found'' and ``a proposal was found but executed poorly'' are scored as the two distinct causal pathways they are, rather than pooled into a single failure category.

\subsection{Number of Grasp Candidates}

Conditional on $\text{has\_grasps} = 1$, the count of valid grasp proposals is modelled as an overdispersed count variable rather than a linear-Gaussian one, since raw candidate counts are strictly non-negative and highly right-skewed:
\begin{equation}
  n_{\text{grasps}} \mid \text{has\_grasps}{=}1 \;\sim\; \text{NegBin}\!\left(\mu = \exp(\zeta_0 + \zeta_1 \sigma_d + \zeta_2 \rho + \zeta_3 \phi + \zeta_4 \theta),\; \kappa\right),
  \label{eq:ng}
\end{equation}
fitted only on the sub-population that clears the gate. This equation, together with Equation~\eqref{eq:hg}, is the hurdle decomposition motivated above: the gate carries the causal claim about \emph{whether} a proposal exists, and Equation~\eqref{eq:ng} carries the separate claim about \emph{how many} exist given that at least one does.

\subsection{Grasp Confidence}

The structural equation for $q_{\text{grasp}}$ models how the CGN confidence score depends on the exogenous variables and on the size of the candidate pool it was drawn from, conditional on $\text{has\_grasps} = 1$:
\begin{equation}
  q_{\text{grasp}} = \gamma_0 + \gamma_1 \sigma_d + \gamma_2 \rho + \gamma_3 \phi + \gamma_4 \theta + \gamma_5 \log(n_{\text{grasps}}) + U_q.
  \label{eq:qg}
\end{equation}
The $\log(n_{\text{grasps}})$ term is an order-statistic correction, not a substantive causal claim about pool size improving quality: $q_{\text{grasp}}$ is defined as the \emph{maximum} score over a pool of $n_{\text{grasps}}$ candidates, so a larger pool mechanically raises the expected maximum even if every individual candidate is drawn from an unchanged distribution. Omitting this term would let $\sigma_d, \rho, \phi, \theta$'s coefficients absorb a pool-size effect that has nothing to do with their causal effect on grasp quality. The noise term $U_q$ captures the residual stochasticity in CGN's scoring not explained by these terms.

\subsection{Grasp Pose Error}

The structural equation for $e_{\text{pose}}$ models the Euclidean distance between the CGN-proposed gripper position and the true object centroid (Section~\ref{sec:variables}), conditional on $\text{has\_grasps} = 1$:
\begin{equation}
  e_{\text{pose}} = \delta_0 + \delta_1 \sigma_d + \delta_2 \rho + \delta_3 \phi + \delta_4 \theta + U_e.
  \label{eq:ep}
\end{equation}
Note that $q_{\text{grasp}}$ does not appear on the right-hand side. Section~\ref{sec:causal_graph} (Assumption 4) establishes from the pipeline's dataflow that $q_{\text{grasp}}$ and $e_{\text{pose}}$ are read from the same $\arg\max$ index of the same underlying CGN output and are therefore correlated siblings, not a cause and its mediator; an earlier version of this equation included $q_{\text{grasp}}$ as a regressor and reported the coefficient as a mediated effect, and that framing is retracted here for exactly this reason (Section~\ref{sec:falsification} reports the residual test that this correction responds to). The direct dependence on $\phi$ and $\theta$ reflects the same camera-to-world transform that determines which surfaces are visible and where a proposed pose lands: certain viewpoints produce systematic biases in CGN's pose estimation. For example, at $\phi = 30°$, CGN consistently proposes positions approximately 6\,cm from the true centroid, even with clean inputs; intervening on $\phi$ changes $e_{\text{pose}}$, so by the definition adopted in Section~\ref{sec:bg_scm} this is a causal effect of viewpoint on pose error, full stop. That the mechanism behind the effect is CGN's training-distribution coverage rather than a defect introduced by the experimental perturbations is an explanation of \emph{why} the effect exists; it does not make the effect any less real or any less a target for the counterfactual attribution in Chapter~\ref{ch:diagnosis}. It does mean that the linear term $\delta_3 \phi$ is not expected to fit this specific viewpoint well: a bias that is large at one elevation and small elsewhere is a threshold effect that a single global slope cannot represent, and Section~\ref{sec:falsification} reports the resulting divergence from the nonparametric estimate directly, rather than concealing it behind a higher-order polynomial chosen after the fact to remove the mismatch. $e_{\text{pose}}$ is also right-skewed in the raw data; fitting it on a log scale was considered as a robustness check and did not change the sign or ranking of any coefficient, so the untransformed scale is retained here for direct interpretability in metres.

\subsection{Binary Outcome}

Unlike the equations above, $Y$ is not fitted as its own regression. $Y = 0$ whenever $\text{has\_grasps} = 0$: with no candidate returned there is no pose to execute, independently of every other variable, and this direct dependence is precisely why $\text{has\_grasps}$ rather than any continuous intermediate is the primary causal claim of Section~\ref{sec:causal_graph}. Conditional on $\text{has\_grasps} = 1$, $Y$ is determined by whether the executed grasp is close enough to the object, operationalised in Section~\ref{sec:success_criterion_detail}; under the historical proximity criterion used to fit the equations above, this reduces to a near-deterministic threshold on $e_{\text{pose}}$ (97.9\% agreement with the logged post-execution outcome, Section~\ref{sec:scm_fit}), which is flagged there as a measurement limitation rather than a causal finding---under that criterion, $Y$ and $e_{\text{pose}}$ are two measurements of nearly the same underlying quantity, not an independent cause-and-effect pair---and is the reason the redesigned floating-gripper outcome (Section~\ref{sec:approach}) is used for the primary results of Chapter~\ref{ch:results} instead. No coefficient $\lambda$ is fitted for $Y$ in this thesis. An earlier draft of this section specified $Y$ as a logistic function of $q_{\text{grasp}}$, $n_{\text{grasps}}$, and $e_{\text{pose}}$ jointly; that equation is retracted here for the reason just given---it would have been fitted on an outcome that is, under the historical criterion, a deterministic function of one of its own inputs, which is exactly the kind of overcommitment the rest of this section tries to avoid.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{5.3cm}p{4.7cm}l}
    ...
    $q_{\text{grasp}} \to e_{\text{pose}}$ (mediation edge, originally pre-registered) & Dataflow reading of \texttt{best\_grasp\_cam}: does the code that computes $e_{\text{pose}}$ read $q_{\text{grasp}}$'s value? & \textbf{Falsified}; both read the same $\arg\max$ index of the same CGN output---siblings, not a mediation chain; edge removed \\
    \bottomrule
  \end{tabular}
\end{table}

Three of the seven pre-specified tests came back against the originally pre-registered graph. The first two concern $C_{pc}$: the original specification asserted that all four exogenous variables affect point cloud completeness (a claim that is mechanistically plausible on its face, since all four describe some aspect of sensing quality); this is wrong for this particular pipeline, because $C_{pc}$ is computed from the segmentation buffer, which is rendered \emph{before} depth noise is injected and \emph{before} the point cloud is downsampled, so $\sigma_d$ and $\rho$ cannot affect it by construction. The data confirm this directly (Table~\ref{tab:falsification}), so the graph (Section~\ref{sec:causal_graph}, Assumption 1) and Equation~\eqref{eq:cpc} were revised to remove both edges rather than retaining them and letting the discrepancy disappear silently into the noise term $U_C$. We report this revision explicitly because it is the clearest available evidence that the graph is a falsifiable claim about this pipeline, not an unfalsifiable restatement of it: it was tested, part of it was wrong, and the model was changed as a result. The $\rho \to \text{has-grasps}$ edge is retained despite a non-significant coefficient at $n=426$, because it is implied directly by the pipeline's dataflow (fewer points can only reduce, never increase, the chance that CGN identifies a graspable region) and by the wider literature on point-cloud density and learned grasp detection; a null result at this sample size is flagged as a limitation on statistical power rather than treated as confirmation of no effect.

The third test that came back against the original specification is the $q_{\text{grasp}} \to e_{\text{pose}}$ mediation edge, and this one is worth stating carefully because a purely statistical test cannot settle it: conditioning on $\{\sigma_d, \rho, \phi, \theta, \log(n_{\text{grasps}})\}$ and regressing $e_{\text{pose}}$ on $q_{\text{grasp}}$ still finds a significant residual coefficient ($n=285$; coefficient $-0.184$, $p=0.015$), which is exactly what a genuine mediation edge \emph{and} a confounded sibling relationship would both produce---a chain and a fork with a shared parent are statistically indistinguishable from conditioning alone. What settles the direction is reading the code that computes each variable (Section~\ref{sec:causal_graph}, Assumption 4): the function that returns $e_{\text{pose}}$'s underlying pose never reads $q_{\text{grasp}}$'s value; both are read from the same $\arg\max$ index of the same scored-candidate output that Contact-GraspNet returns for the trial. The residual correlation is confirmed, but it is confirmed as evidence \emph{for} the sibling reading, not for the originally pre-registered mediation edge, which is removed. This is reported here as a second, independent instance (beyond the $C_{pc}$ correction) of the graph being wrong in a way the data could have caught, and correcting it rather than retaining a plausible-sounding but dataflow-unsupported causal story.
...
Three aspects of this definition are worth making explicit rather than leaving implicit. First, it is anchored to a specific \emph{intervention magnitude}: a full reset of one variable to its nominal clean-baseline value, not a partial or graded correction. The reported irreducibility rate is therefore a statement about failures that resist this particular class of intervention, not about irreducibility in some magnitude-independent sense; Chapter~\ref{ch:discussion} returns to how sensitive the 57.2\% figure is to this choice. Second, the nominal value chosen for $\phi$ (45°, rather than the other two sampled elevations) is itself a modelling choice the 57.2\% figure depends on: a failure attributed to $\phi$ under this baseline could in principle be classified differently under $\phi_{\text{nom}} = 30°$, and Chapter~\ref{ch:discussion} flags this baseline-sensitivity alongside the magnitude-sensitivity above, rather than treating only the latter as a limitation. Third, the definition is evaluated against simulated ground truth (Section~\ref{sec:counterfactual_diagnosis} details how $Y(v \!\to\! v_{\text{nom}})$ is obtained, either by re-simulation with the trial's seed held fixed---physical abduction, Section~\ref{sec:approach}---or by the fitted surrogate SCM), not against the SCM's own prediction of itself, which is what keeps the resulting classification from being tautological. Reading a point counterfactual off the surrogate route, rather than the re-simulation route, additionally requires a rank-preservation assumption on the abducted noise term (Section~\ref{sec:counterfactual_diagnosis}): that the trial's position in the noise distribution, once solved for under the observed condition, is preserved under the intervened-on condition. Where re-simulated ground truth is available it is used in preference to the surrogate for exactly this reason.
...
\begin{hypothesis}
  Counterfactual diagnosis over the fitted SCM will attribute single-cause grasp failures to the correct root variable in more than 50\% of test cases---a bar chosen because chance attribution among the four candidate variables is approximately 25\%.
  \label{hyp:accuracy}
\end{hypothesis}
...
Each confirmatory trial proceeds through the following stages:

\begin{enumerate}
  \item \textbf{Scene initialisation.} Load the object-specific MuJoCo scene, reset to the home keyframe, and run \texttt{mj\_forward}.

  \item \textbf{Camera positioning.} Place the virtual camera at $(\phi, \theta)$ on a fixed-radius sphere (0.8\,m) around the target centroid.

  \item \textbf{Depth rendering and noise injection.} Render the depth buffer at $480 \times 640$ and inject $\mathcal{N}(0, \sigma_d^2)$ per pixel; clip to non-negative values.

  \item \textbf{Segmentation.} Render the segmentation buffer and compute $C_{pc}$ as the fraction of target pixels.

  \item \textbf{Point cloud generation.} Back-project the noisy depth map; randomly downsample to fraction $\rho$.

  \item \textbf{Grasp synthesis.} Feed the degraded cloud to Contact-GraspNet; record the top-ranked 6-DoF pose, $q_{\text{grasp}}$, and $n_{\text{grasps}}$.

  \item \textbf{Frame transformation.} Convert the proposal from the camera frame to the MuJoCo world frame.

  \item \textbf{Variable measurement.} Compute $e_{\text{pose}}$ as the Euclidean distance between the proposed position and the true object centroid (logged as a mediator, not as the outcome).

  \item \textbf{Grasp execution.} Evaluate the proposal with the floating-gripper protocol of Section~\ref{sec:grasp_execution} (open-hand collision check, finger closure, lift/shake).

  \item \textbf{Outcome evaluation.} Set $Y=1$ iff the object remains in the gripper footprint and achieves the lift threshold throughout the disturbance window (Section~\ref{sec:success_criterion_detail}).

  \item \textbf{Data logging.} Record the exogenous settings, mediators, collision flag, and $Y$ to CSV.
\end{enumerate}
...
\textbf{Sparsity ($\rho$).} Uniform progression from full density to 75\% random drop. Independent Bernoulli retention keeps $\rho$ unconfounded with the other factors.
...
The filtered arm isolates the gate. At one post-hoc clean viewpoint, top-$k$ collision rejection ($k=20$) reaches 89\% under clamp-and-lift, with every remaining failure a residual collision. The matched unfiltered cell is 2/5; cylinder top-1 across clean viewpoints is 10.0\%. The 89\% figure is a ceiling, not an operating rate, and it uses a lighter criterion than the main-grid shake. It does show that the headline 5.0\% is dominated by executing the top-ranked colliding pose, not by an inability of the weld-hand to lift a geometrically valid grasp.
...
\begin{enumerate}
  ...
  \item \textbf{Floating-gripper idealisation.} The test assumes the end-effector can be held at any commanded pose. Unreachable or self-colliding proposals for the full manipulator are not penalised. Success rates are not full-system pick rates. Full-arm due diligence later showed that reach can be solved (25/25 at one clean cell) while frictional lift on those Contact-GraspNet poses remains 0/25 unless forearm collisions are ghosted, geometry is thinned, friction is raised, and---for a held lift---a weld is latched (Appendix~\ref{sec:arm_v4}).
