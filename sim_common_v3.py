...
sim_common_v3.py
================
v3 copy of sim_common.py — do NOT edit the original sim_common.py.

Adds clean-condition helpers used by run_experiments_v3.py:
  - ranked_grasps_overall / top-k collision filtering
  - probe_pregrasp_collision
  - run_floating_gripper_lift_test (clamp + lift only; no XY shake)

Preserves all v2 helpers so v3 can import this module alone.
"""
...
def ranked_grasps_overall(pred_grasps, scores, top_k=10):
    """Return up to ``top_k`` grasps across all segments, highest score first.

    Each entry is ``(pose_cam_4x4, score, seg_id)``. Used by the v3
    ACRONYM-style top-k + collision-filter selection gate.
    """
    candidates = []
    for seg_id in pred_grasps:
        s, g = scores[seg_id], pred_grasps[seg_id]
        ...
        for i in range(len(s)):
            candidates.append((g[i].copy(), float(s[i]), seg_id))
...
def probe_pregrasp_collision(model, data, grasp_pos, grasp_quat,
                              hand_body='hand', gripper_bodies=GRIPPER_BODIES):
    """Teleport open gripper to pose; return (collision_free, contacted_bodies)."""
    teleport_hand_hard(model, data, grasp_pos, grasp_quat, hand_body=hand_body)
    open_gripper(model, data)
    mujoco.mj_forward(model, data)
    contacted = sorted(gripper_contacted_bodies(model, data, gripper_bodies))
    return len(contacted) == 0, contacted


def run_floating_gripper_lift_test(model, data, target_body_name, grasp_pos, grasp_quat,
                                    footprint_radius, hand_body='hand',
                                    gripper_bodies=GRIPPER_BODIES,
                                    close_steps=400, lift_steps=400,
                                    lift_height=0.15,
                                    xy_tolerance_margin=0.03, sample_every=25,
                                    squeeze_margin=60.,
                                    min_lift_frac=0.4,
                                    skip_pregrasp_check=False):
    """
    Lighter v3 success criterion: clamp + vertical lift only (no XY shake).

    Success = object stays within the gripper XY footprint AND is lifted
    at least ``min_lift_frac * lift_height`` by the end of the lift ramp
    (default 40% of 15 cm ≈ 6 cm off the table). Sufficient for reporting
    clean-condition grasp rates without the full ACRONYM shake disturbance.
    """
    teleport_hand_hard(model, data, grasp_pos, grasp_quat, hand_body=hand_body)
    open_gripper(model, data)
    mujoco.mj_forward(model, data)
    ...
    if not collision_free and not skip_pregrasp_check:
        return result
    ...
    for i in range(lift_steps):
        t = i / max(1, lift_steps)
        ramp = min(1.0, t / 0.3)
        dz = lift_height * ramp
        teleport_mocap(model, data, 'hand_target',
                       grasp_pos + np.array([0.0, 0.0, dz]), grasp_quat)
        mujoco.mj_step(model, data)
        if i % sample_every == 0 and ramp >= 1.0:
            obj_pos = data.xpos[obj_id]
            xy_off = float(np.linalg.norm(obj_pos[:2] - data.site_xpos[ee_site][:2]))
            if xy_off > tol or (obj_pos[2] - z0) < lift_height * min_lift_frac:
                held_throughout = False

    obj_pos_final = data.xpos[obj_id].copy()
    xy_offset = float(np.linalg.norm(obj_pos_final[:2] - data.site_xpos[ee_site][:2]))
    lift = float(obj_pos_final[2] - z0)
    success = held_throughout and xy_offset <= tol and lift >= lift_height * min_lift_frac
...
def execute_topk_collision_filtered_lift(
        model, data, target_body_name, obj_pos, obj_quat,
        ranked_world_grasps, footprint_radius,
        close_steps=400, lift_steps=400, lift_height=0.15,
        xy_tolerance_margin=0.03, settle_steps=30):
    """
    ACRONYM-style gate: walk ranked world grasps (pos, quat, score) in order;
    skip any open-finger pre-grasp that contacts object/table; execute the
    first collision-free pose with clamp+lift.

    ``ranked_world_grasps``: list of ``(grasp_pos, grasp_quat, score)``.

    Returns the lift-test result dict plus:
      selected_rank (0-based index among candidates tried, or None),
      selected_score, n_candidates_tried, n_rejected_collision.
    """
    n_tried = 0
    n_rejected = 0
    last_collision_result = None

    for rank, (grasp_pos, grasp_quat, score) in enumerate(ranked_world_grasps):
        set_object_pose(model, data, target_body_name, obj_pos, obj_quat)
        mujoco.mj_forward(model, data)
        settle(model, data, settle_steps)

        collision_free, contacted = probe_pregrasp_collision(
            model, data, grasp_pos, grasp_quat)
        n_tried += 1
        if not collision_free:
            n_rejected += 1
            last_collision_result = dict(
                collision_free=False, contacted_bodies=contacted,
                success=False, final_xy_offset=None, final_lift=None,
                obj_z_final=None,
                selected_rank=None, selected_score=None,
                n_candidates_tried=n_tried, n_rejected_collision=n_rejected,
                attempted_score=float(score), attempted_rank=rank)
            continue

        # Collision-free: run lift test (skip re-check; pose already probed)
        set_object_pose(model, data, target_body_name, obj_pos, obj_quat)
        mujoco.mj_forward(model, data)
        settle(model, data, settle_steps)
        result = run_floating_gripper_lift_test(
            ...
            skip_pregrasp_check=True)
        # Probe already established open-finger clearance; keep that label
        # even if the second teleport re-measures contacts during close.
        result['collision_free'] = True
        result['contacted_bodies'] = []
        result.update(
            selected_rank=rank, selected_score=float(score),
            n_candidates_tried=n_tried, n_rejected_collision=n_rejected,
            attempted_score=float(score), attempted_rank=rank)
        return result

    # All candidates collided (or list empty)
    if last_collision_result is not None:
        return last_collision_result
    return dict(
        collision_free=False, contacted_bodies=[],
        success=False, final_xy_offset=None, final_lift=None, obj_z_final=None,
        selected_rank=None, selected_score=None,
        n_candidates_tried=0, n_rejected_collision=0,
        attempted_score=None, attempted_rank=None)
