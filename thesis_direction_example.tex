...
\begin{itemize}
  \item $\sigma_d \in \{0,\ 0.01,\ 0.02,\ 0.04\}$\,m: standard
        deviation of Gaussian depth noise injected per pixel.
  \item $\rho \in \{1.0,\ 0.5,\ 0.25\}$: fraction of point cloud
        retained after random downsampling.
  \item $\phi \in \{30°,\ 45°,\ 60°,\ 75°\}$: camera elevation
        on a fixed-radius sphere centred on the object.
  \item $\theta \in \{0°,\ 60°,\ 120°\}$: camera azimuth on the
        same sphere.
\end{itemize}

\paragraph{Endogenous variables (measured per trial)}
\begin{itemize}
  \item $C_{pc}$: point cloud completeness --- ratio of observed
        surface points to full ground-truth mesh surface.
  \item $q_{\text{grasp}}$: Contact-GraspNet top-1 confidence score
        for the selected grasp pose.
  \item $e_{\text{pose}}$: grasp pose error --- Frobenius norm of
        the transform difference between the proposed and oracle pose.
  \item $Y \in \{0,1\}$: binary grasp success (object held after
        lift of 0.15\,m and hold of 1.0\,s).
\end{itemize}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node (cpc)  [right=30mm of rho]    {$C_{pc}$};
    \node (qg)   [right=30mm of cpc, yshift=8mm]  {$q_{\text{grasp}}$};
    \node (ep)   [right=30mm of cpc, yshift=-8mm] {$e_{\text{pose}}$};
    \node (Y)    [right=30mm of qg, yshift=-8mm]  {$Y$};
  ...
  \caption{Causal DAG. Exogenous variables ($\sigma_d$, $\rho$,
           ...
           ($C_{pc}$, $q_{\text{grasp}}$, $e_{\text{pose}}$) to
           binary grasp outcome $Y$.}
...
\begin{align}
  ...
  P(Y{=}1)         &= \sigma\!\left(\lambda_0
                      ...
                      + \lambda_2 e_{\text{pose}}\right), \label{eq:Y}
\end{align}
where $\sigma(\cdot)$ is the logistic function and
$U_C$, $U_q$, $U_e$ are exogenous noise terms absorbed during
the Abduction step.
...
\begin{enumerate}
  \item Position the camera at $(\phi, \theta)$ on a fixed-radius
        sphere around the object centroid.
  \item Render the MuJoCo depth buffer and inject
        $\mathcal{N}(0, \sigma_d^2)$ noise per pixel; clip
        to non-negative values.
  \item Back-project the noisy depth map to a 3D point cloud;
        randomly downsample by fraction $\rho$.
  \item Feed the degraded point cloud to Contact-GraspNet and
        record the top-ranked grasp pose and confidence
        score~$q_{\text{grasp}}$.
  \item Measure $C_{pc}$ and $e_{\text{pose}}$ (versus the
        oracle pose recorded under clean conditions).
  \item Execute the grasp via inverse kinematics in MuJoCo;
        lift 0.15\,m; record $Y$.
  \item Log $(\sigma_d, \rho, \phi, \theta, C_{pc},
        q_{\text{grasp}}, e_{\text{pose}}, Y)$ to CSV.
\end{enumerate}
...
All experiments are conducted in \textbf{MuJoCo}~\cite{todorov2012mujoco}.
The tabletop scene includes a Franka Panda arm (from MuJoCo
Menagerie~\cite{menagerie2022}), a target object placed on a table surface,
and a virtual RGB-D camera positioned on a fixed-radius sphere centred on
the object.  The camera radius is fixed at 0.8\,m, consistent with the
operating range of commodity depth sensors mounted on a robot or a
laboratory stand~[CITE: RealSense D435 datasheet].
...
\begin{table}[t]
  \caption{Experimental Variable Grid}
  \label{tab:grid}
  \centering
  \begin{tabular}{llr}
    \toprule
    ...
    \midrule
    $\sigma_d$ (m)      & 0, 0.01, 0.02, 0.04 & 4 \\
    $\rho$              & 1.0, 0.5, 0.25      & 3 \\
    $\phi$ (elevation)  & 30°, 45°, 60°, 75°  & 4 \\
    $\theta$ (azimuth)  & 0°, 60°, 120°       & 3 \\
    \midrule
    Full factorial      & $4{\times}3{\times}4{\times}3$ & 144 \\
